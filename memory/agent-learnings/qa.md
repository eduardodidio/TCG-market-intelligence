...
- **Verify that custom CSS class names used in components are defined in the Tailwind config.** OBS-F53-01 caught `animate-fade-in-up` used in GauchoDialog but never defined in `tailwind.config.ts`. Tailwind silently ignores unknown utility classes, so this kind of bug produces no error -- the animation simply does not render. For any component using a non-standard Tailwind class (anything not in the default Tailwind palette), grep the tailwind config to confirm the class is defined. This is especially important for custom animations, colors, and spacing values.
- **AbortSignal forwarding is a recurring frontend bug pattern.** F52-T01's root cause was that `TrendingSection` called `fetchTrending()` without passing the `signal` from `useApi`, causing silently aborted requests. When reviewing any `useApi` consumer, verify that the signal parameter is threaded through to the underlying fetch call. The pattern `(signal) => fetchFn(params, { signal })` must be used consistently -- omitting it causes intermittent data disappearance that is nearly impossible to reproduce in testing.
- **Cache invalidation must be verified across all cache layers.** F52-T02 added trending cache invalidation via `ScanHookRegistry`, complementing the existing market data cache invalidation. When multiple cache layers exist (here: AggregateCache, TrendingService._cache, endpoint caches), every scan-triggered invalidation must touch all relevant caches. QA should maintain a list of all cache layers and verify each has a corresponding invalidation hook.

## F54 -- Trending List Layout + Gaucho Orthography + Ticker Animation (2026-08-23)

- **For i18n string fixes, verify every corrected string against the spec table character-by-character.** F54-T02 had 9 string corrections involving accented characters (a with accent, e with accent, o with accent, a-tilde-o). A quick visual scan can miss subtle differences between accented and unaccented characters. Compare each key's value in both locale files against the spec table entry by entry, and confirm both locale files are identical for the shared section.
- **Backward compatibility of shared components is a first-priority QA check.** When a shared component gains a new prop (like `TrendingSection`'s `variant`), immediately grep for all consumers and verify none pass the new prop unexpectedly. The default value must preserve existing behavior. This is a 30-second check that prevents the highest-impact regressions.
- **CSS animation changes require formula verification with boundary values.** The ticker speed formula `max(10, items.length)` has two boundary behaviors: (1) items <= 10 are clamped to 10s, (2) items > 10 scale linearly. QA should verify both the formula and the test assertions match, especially the clamping behavior at the minimum.

## F65 -- Credit Token System (2026-08-26)

- **When a feature adds a confirmation step (modal/dialog) before an existing action, immediately audit ALL tests that click the original trigger.** F65 added a CreditConfirmModal before refresh buttons. 7 existing Liga tests broke because they clicked the button expecting an immediate API call. The fix required mocking the credits balance endpoint and adding a modal-confirm step. Before declaring a modal/dialog feature complete, grep for every `data-testid` of the wrapped button and verify each test accounts for the intermediate step.
- **Pre-existing test regressions from uncommitted changes compound during QA.** F65 QA encountered 3 test failures from prior commits (seed user removal, CLI rename, default password change) that were unrelated to F65 but blocked the full test suite from passing. QA should always run the full test suite BEFORE starting feature validation to distinguish pre-existing failures from feature regressions.
- **Verify backend response field names against frontend TypeScript interfaces as a dedicated QA check.** B1 (field name mismatch on claim-bonus) was caught by TechLead review, not by tests. TypeScript interfaces do not fail at build time when a JSON response field is missing -- the value silently becomes `undefined`. QA should add a checklist item: "for each new API endpoint, verify the frontend type interface field names match the backend response keys exactly."

## F124 -- Collection Valuation Panel & Liga Link Fix (2026-09-13)
**What worked:** Sharded brief + wave-summary carry-forward reading was sufficient to validate all 7 global ACs without opening the full PRD/ADR/brief — task/wave summaries plus direct test-file inspection covered it.
**What to avoid:** Don't accept a Developer/TechLead summary's claim that failing tests are "pre-existing" at face value when signing off a PASSED verdict.
**Pattern to repeat:** Verify "pre-existing failure" claims independently via `git stash` + re-running the suite on the unmodified baseline before including them in the QA report. This is cheap and catches cases where a claim is stale or wrong.
