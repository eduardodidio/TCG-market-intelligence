...
- **Place cross-cutting service features (F44) in the wave AFTER the services they will unify, not before.** F44 (shared data arch) was correctly placed in Wave 2, after F32/F33/F36 had each independently implemented their own caching and data access patterns. This allowed F44 to observe the natural patterns that emerged and consolidate them, rather than pre-designing abstractions that might not fit. When planning a "shared infrastructure" feature, let the concrete implementations exist first, then extract.
- **Four waves with 3-4 features each is the right granularity for a 13-feature phase.** P2 had zero dependency violations and zero backport fixes across 4 waves. The key was ensuring each wave's outputs were stable inputs for the next wave. Wave 0 (data foundations) -> Wave 1 (analytics) -> Wave 2 (services + cross-cutting) -> Wave 3 (UI composition) is a reusable template for future phases.
- **Documentation requirements must be planned as explicit tasks, not assumed.** P2 shipped 13 features with zero PRDs and only 10 of 26 required diagrams. The CLAUDE.md rules were not enforced because the Architect did not create explicit documentation tasks in the wave plans. For future phases, include "Create PRD" as a Wave 0 task and "Create diagrams + update README" as a final-wave task for every feature.
- **When multiple features will need caching, plan a unified cache module in an early wave.** P2 ended up with three independent caching implementations (TrendingService dict, market.py _endpoint_cache, AggregateCache) because caching was an afterthought. If F44's AggregateCache had been planned in Wave 0, the other features would have adopted it from the start. Shared infrastructure decisions should be identified during phase planning, not discovered during refactoring.
- **Allocate a dedicated "presentation composition" wave at the end.** Wave 3 (F38 landing, F39 ticker, F40 market page) was purely UI composition -- no new data models, no new backend logic, just assembling existing APIs into pages. This wave had the lowest risk and highest user-visible impact. It is the right place to be in the final wave.
