...
## Testing
- Docs-only.
- Validar sintaxe Mermaid: se `npx @mermaid-js/mermaid-cli` não estiver instalado, **não instalar** (sem deps novas) — revisar manualmente contra os templates.
...
**Test plan:** ver `F176-test-plan.md` (fixtures: nenhum — docs-only, sem fixtures de teste)
...
- ADR number **0017** (batch reservation in `EXECUTION-PLAN-F171-F179.md` overrides "next free"; 0014 was taken by F171, 0019 by the news feature). `F176-README.md` links it.
- Backfill marker recorded in the ADR as `source='daily_snapshot_backfill'` (Governance amendment 5). T05/T03 run in parallel: if T05 ships a different marker, T05 updates the ADR's "Backfill marker" paragraph; T03's resolver must treat the marker like `daily_snapshot` (priority 9, same per-variant keys).
- Diagnosis confirmed H1–H6 (H3 confirmed in code, inconclusive for real data) and did not refute H4. No change to the key rules; clarifications added as a note at the top of `_brief/02-key-resolution.md`.
- Follow-ups recorded in the ADR: unify `low`/`mid` on `liga_{id}` (refresh/scan vs sweep); `get_price_series_batch` has the same H1 defect (sparklines, deck valuation).
- Mermaid CLI not installed (no new deps). Diagrams reviewed by hand against the templates; every label containing `{}`, `()` or `>` is quoted.
