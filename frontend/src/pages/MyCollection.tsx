      ...
      >
        ...
        <div className="sm:hidden flex justify-end items-center gap-3">
          <div className="relative" ref={actionsDropdownRef}>
            <button
              ...
              className="px-3 py-2 text-sm font-medium bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-md transition-colors flex items-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
            ...
            {actionsOpen && (
              <div className="absolute right-0 top-full mt-1 bg-slate-800 border border-slate-600 rounded-lg shadow-lg z-20 p-2 min-w-48 space-y-1" data-testid="mobile-actions-menu">
                {/* Select mode toggle */}
                <button
                  type="button"
                  onClick={() => {
                  ...
                  data-testid="mobile-select-mode-btn"
                >
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                {selectionMode && (
                  <button
                    type="button"
                    onClick={() => {
                      (selectedCount === cards.length ? deselectAll : handleSelectAll)();
                      setActionsOpen(false);
                    }}
                    className="w-full text-left px-3 py-2 text-sm text-cyan-400 hover:bg-slate-700 rounded transition-colors"
                    data-testid="mobile-select-all-btn"
                  >
                ...
                <button
                  type="button"
                  onClick={() => { setCsvImportOpen(true); setActionsOpen(false); }}
                  className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm text-slate-300 hover:bg-slate-700 rounded transition-colors"
                  data-testid="mobile-import-csv-btn"
                >
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <button
                  type="button"
                  onClick={() => { setBatchAddOpen(true); setActionsOpen(false); }}
                  className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm text-slate-300 hover:bg-slate-700 rounded transition-colors"
                  data-testid="mobile-add-cards-btn"
                >
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                {/* Bulk canonize */}
                {summary && (summary.total_unique - summary.linked_count) > 0 && (
                ...
                {!isRefreshing && !refreshDone && !refreshError && (
                  <button
                    type="button"
                    onClick={() => { handleRefreshAllClick(); setActionsOpen(false); }}
                    className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm text-slate-300 hover:bg-slate-700 rounded transition-colors"
                    data-testid="mobile-refresh-all-btn"
                  >
                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                    </svg>
                    {t("collection.refreshAll")}
                ...
                {!isRefreshing && !refreshDone && !refreshError && summary && summary.total_unique > 0 && (
                  <button
                    type="button"
                    onClick={() => { setQueueRefreshModalOpen(true); setActionsOpen(false); }}
                    disabled={queueRefreshLoading}
                    className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm text-slate-300 hover:bg-slate-700 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    data-testid="mobile-queue-refresh-all-btn"
                  >
                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l4 2m6-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    {t("collection.queueRefreshAll")}
        ...
        <div className="hidden sm:flex justify-end items-center gap-3">
          ...
          >
            ...
            <span>{t("collection.importCsv")}</span>
          </button>
          ...
          {!isRefreshing && !refreshDone && !refreshError && (
            ...
            >
              ...
              <span>{t("collection.refreshAll")}</span>
              {summary && summary.total_unique > 0 && (
                <CostBadge cost={summary.total_unique} balance={creditBalance} />
              )}
            </button>
          ...
          {!isRefreshing && !refreshDone && !refreshError && summary && summary.total_unique > 0 && (
            ...
            >
              ...
              <span>{t("collection.queueRefreshAll")}</span>
              <CostBadge cost={Math.min(summary.total_unique, 500)} balance={creditBalance} />
            </button>
          )}
